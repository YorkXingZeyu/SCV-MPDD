...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--freeze_unet_lora",
        action="store_true",
        help="Ablation: freeze UNet LoRA at init (zero-init B -> UNet stays base). Trains TE-LoRA only.",
    )
    parser.add_argument(
        "--freeze_text_lora",
        action="store_true",
        help="Ablation: freeze text-encoder LoRA at init (zero-init B -> TE stays base). Trains UNet-LoRA only.",
    )
...
def main(args):
    ...
    # ----- Ablation: optionally freeze one LoRA branch at its (zero-init) state -----
    # PEFT zero-inits lora_B, so a frozen LoRA branch is a no-op adapter and the
    # underlying component behaves exactly as the pretrained base. The saved format
    # stays identical (both unet/ and text_encoder/ adapters exist) -> eval unchanged.
    if args.freeze_unet_lora:
        for n, p in unet.named_parameters():
            if "lora_" in n:
                p.requires_grad_(False)
        logger.info("[ablation] UNet LoRA frozen at init (TE-LoRA-only run).")
    if args.freeze_text_lora:
        for n, p in text_encoder.named_parameters():
            if "lora_" in n:
                p.requires_grad_(False)
        logger.info("[ablation] Text-encoder LoRA frozen at init (UNet-LoRA-only run).")
    ...
    # Optimizer creation (only LoRA params that require grad; frozen branch excluded)
    optimizer = optimizer_class(
        [
            {"params": [p for p in unet.parameters() if p.requires_grad], "lr": args.unet_learning_rate},
            {"params": [p for p in text_encoder.parameters() if p.requires_grad], "lr": args.text_encoder_learning_rate}
        ],
